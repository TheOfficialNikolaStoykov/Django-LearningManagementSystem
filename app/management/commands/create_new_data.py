...
class Command(BaseCommand):
    ...
    def delete_migrations(self):
        ...
        for root, dirs, files in os.walk(migrations_path, topdown=False):
            for name in files:
                if name != '__init__.py':
    ...
    def create_teacher_users(self):
        CustomUser.objects.create_user(username='vvelichkov', password='vvelichkovlms123@@@$$$%%%', is_teacher=True)
        CustomUser.objects.create_user(username='itodorova', password='itodorovalms123@@@$$$%%%', is_teacher=True)
        CustomUser.objects.create_user(username='mevgeniev', password='mevgenievlms123@@@$$$%%%', is_teacher=True)
        CustomUser.objects.create_user(username='mstoyanova', password='mstoyanovalms123@@@$$$%%%', is_teacher=True)
        CustomUser.objects.create_user(username='lbalkanski', password='lbalkanskilms123@@@$$$%%%', is_teacher=True)

    
    def create_student_users(self):
        CustomUser.objects.create_user(username='svladov', password='svladovlms123@@@$$$%%%', is_student=True)

    def create_teacher_accounts(self):
        teachers_create_list = [Teacher(first_name='Ventsislav', last_name='Velichkov', date_of_birth='1989-01-24', address='10, ul. Vihren', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='vvelichkov')),
                                Teacher(first_name='Iliyana', last_name='Todorova', date_of_birth='1993-09-21', address='70-72, bul. Cherni vrah', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='itodorova')),
                                Teacher(first_name='Milen', last_name='Evgeniev', date_of_birth='1983-07-31', address='25, ul. Asenova krepost', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='mevgeniev')),
                                Teacher(first_name='Marina', last_name='Stoyanova', date_of_birth='1993-04-04', address='43А, ul. Todor Dzhebarov', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='mstoyanova')),
                                Teacher(first_name='Lyubomir', last_name='Balkanski', date_of_birth='1986-09-04', address='19А, ul. Boris Rumenov', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='lbalkanski'))]

        Teacher.objects.bulk_create(teachers_create_list)

    def create_student_account(self):
        students_create_list = [Student(degree='BA', faculty_id='540029', first_name='Stoyan', last_name='Vladov', date_of_birth='1998-03-12', address='20, ul. Prof. d-r Ivan Stranski', email='nikola.stoykov666@gmail.com', user=CustomUser.objects.get(username='svladov'))]
        Student.objects.bulk_create(students_create_list)

    def create_courses(self):
        svladov_user = CustomUser.objects.get(username='svladov')
        
        svladov_student = Student.objects.get(user=svladov_user)
        
        course_maths = Course.objects.create(name='Mathematics', teacher=Teacher.objects.get(last_name='Velichkov'))
        course_maths.students.add(svladov_student)

        course_econ = Course.objects.create(name='Economics', teacher=Teacher.objects.get(last_name='Todorova'))
        course_econ.students.add(svladov_student)

        course_soft_dev = Course.objects.create(name='Software Development', teacher=Teacher.objects.get(last_name='Evgeniev'))
        course_soft_dev.students.add(svladov_student)

        course_comp_netw = Course.objects.create(name='Computer Networks', teacher=Teacher.objects.get(last_name='Stoyanova'))
        course_comp_netw.students.add(svladov_student)
        ...
        course_mob_apps.students.add(svladov_student)

    def create_sections(self):
        mathematics = Course.objects.get(name='Mathematics')
        economics = Course.objects.get(name='Economics')
        software_dev = Course.objects.get(name='Software Development')
        computer_networks = Course.objects.get(name='Computer Networks')
        mobile_apps = Course.objects.get(name='Mobile Applications Development')

        sections_create_list = [
            Section(title='Algebra', course=mathematics),
            Section(title='Calculus', course=mathematics),
            Section(title='Geometry', course=mathematics),
            Section(title='Probability and Statistics', course=mathematics),
            Section(title='Set Theory', course=mathematics),
            Section(title='Trigonometry', course=mathematics),
            Section(title='Labour Economics', course=economics),
            Section(title='Econometrics', course=economics),
            Section(title='Financial Economics', course=economics),
            Section(title='Health Economics', course=economics),
            Section(title='Macroeconomics', course=economics),
            Section(title='Microeconomics', course=economics),
            Section(title='Introduction to Python', course=software_dev),
            Section(title='Strings, Lists and Tuples', course=software_dev),
            Section(title='Dictionaries and Sets', course=software_dev),
            Section(title='Conditional Execution & Loops', course=software_dev),
            Section(title='Comprehensions', course=software_dev),
            Section(title='Functions', course=software_dev),
            Section(title='Network Basics', course=computer_networks),
            Section(title='TCP/IP Model', course=computer_networks),
            Section(title='OSI Model', course=computer_networks),
            Section(title='Routing', course=computer_networks),
            Section(title='IP Addressing', course=computer_networks),
            Section(title='Network Services', course=computer_networks),
            Section(title='Class and Object', course=mobile_apps),
            Section(title='Nested and Inner Class', course=mobile_apps),
            Section(title='Kotlin Constructor', course=mobile_apps),
            Section(title='Visibility Modifier', course=mobile_apps),
            Section(title='Kotlin Inheritance', course=mobile_apps),
            Section(title='Abstract Class', course=mobile_apps)
        ]
    ...
    def handle(self, *args, **options):
        ...
        self.create_lessons()
        self.create_projects_in_wistia()
        self.upload_videos_to_wistia()
        self.create_news()
